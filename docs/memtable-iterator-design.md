...
- 对应接口：`Memtable.Iter()` / `Memtable.IterRange(...)`，返回 `Iterator`（拷贝输出）。

2. Flush/Compaction 路径（性能优先）
- 后台流程希望尽量减少分配和拷贝，用顺序流方式写 SST 或做 merge。
- 对应接口：`FlushIterable.RawIter()` / `FlushIterable.RawIterRange(...)`，返回 `RawIterator`（零拷贝输出）。
...
接口定义位于：
- [`/Users/a58/myProject/lsm-tree/internal/memtable/interfaces.go`](/Users/a58/myProject/lsm-tree/internal/memtable/interfaces.go)
...
### 2.3 FlushIterable + RawIterator（零拷贝）

`FlushIterable` 在 `Memtable` 之上额外暴露：
- `RawIter() RawIterator`
- `RawIterRange(start, end []byte) RawIterator`

`RawIterator` 与 `Iterator` 方法签名相同，但语义不同：
- `Key/Value` 指向内部缓冲区（零拷贝）。
- 调用方不得修改。
- 不得在 `Next/Close` 后继续持有引用。

保留 `FlushIterable` 的意义是能力隔离：普通调用方只看见安全接口，后台路径显式断言后才能用 raw 能力。
...
1. 创建：`it := mt.Iter()` 或 `it := fi.RawIterRange(...)`
2. 循环：
...
如果把 raw 能力直接并入 `Memtable` 主接口，会让普通调用方更容易误用（改写内部数据或持有失效引用）。
